...
def run_beam_search(sess, trans_system, model, feat_vocab, action_vocab, batch, cache_size, options):
    ...
    while steps < options.max_answer_len and len(results) < options.beam_size:
        ...
        for i in xrange(cur_size):
            ...
            for j in xrange(min(options.topk_size, topk_ids.shape[-1])):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ulf", action="store_true", help="if this is for ULFs.")
    ...
    if use_dep:
        testset = NP2P_data_stream.read_Testset(in_path, decode=True, ulfdep=args.ulf)
    elif FLAGS.infile_format == 'fof':
        testset = NP2P_data_stream.read_generation_datasets_from_fof(in_path, isLower=FLAGS.isLower, ulfdep=args.ulf)
    else:
        testset = NP2P_data_stream.read_all_GenerationDatasets(in_path, isLower=FLAGS.isLower, ulfdep=args.ulf)
