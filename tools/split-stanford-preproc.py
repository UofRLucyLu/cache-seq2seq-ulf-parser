...
jd = json.loads(file(sys.argv[1], 'r').read())
...
tout = file(os.path.join(sys.argv[2], TOKEN_FILE), 'w')
tout.write(u"\n".join(tokens).encode("utf-8"))
tout.close()
pout = file(os.path.join(sys.argv[2], POS_FILE), 'w')
pout.write(u"\n".join(poses).encode("utf-8"))
pout.close()
lout = file(os.path.join(sys.argv[2], LEMMA_FILE), 'w')
lout.write(u"\n".join(lemmas).encode("utf-8"))
lout.close()
nout = file(os.path.join(sys.argv[2], NER_FILE), 'w')
