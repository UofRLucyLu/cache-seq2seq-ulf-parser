...
python3 data_processing/ulf_align.py ${ULF_DATA} ${ULF_DATA}/amr ${ULF_DATA}/alignment.amr xiaochang t 
...
# Reformatting
python3 data_processing/prepareTokens.py --task categorize --data_dir ${ULF_DATA} --use_lemma --run_dir ${CONLL_GEN} --stats_dir ${CONLL_GEN}/stats --conll_file ${CONLL_GEN}/amr_conll --table_dir ${ULF_DATA}/tables
...
python3 data_processing/ulf_atom_counts.py  --annsent_dir ulfdata/${TRAIN_VER} \
  --alignment_file ulfdata/${TRAIN_VER}/conll/alignment \
  --outfile ulf_atom_counts/${TRAIN_VER}.ua_count
python3 data_processing/predict_ulf_atoms.py --annsent_dir ${ULF_DATA} \
...
python3 ./oracle/oracle.py --data_dir ${CONLL_GEN} --output_dir ${ORACLE_DIR} --cache_size ${CACHE_SIZE} --ulf
# Generate decoding...
"""
