...
def read_all_GenerationDatasets(inpath, isLower=True, dep_path=None, token_path=None, ulfdep=False):
...
def read_Testset(indir, isLower=True, decode=False, ulfdep=False):
    if decode:
...
class DataStream(object):
    def __init__(self, all_questions, word_vocab=None, char_vocab=None, POS_vocab=None, feat_vocab=None, action_vocab=None,
        ...
        for (input_sent, concepts, cid2wid, feats, actions, action2cid, action2wid) in all_questions:# sent1 is the long passage or article
            ...
            for x in feats_idx:
                assert len(x) == options.feat_num, (len(x), options.feat_num)
