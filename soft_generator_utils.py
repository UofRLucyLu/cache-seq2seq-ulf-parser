...
class AttnGen:
    ...
    def one_step_decoder(self, state_t_1, context_input_t_1, context_concept_t_1, word_t, featidx_t,
        ...
        print("feat_num {}\tfeat_dim {}".format(options.feat_num, options.feat_dim))
        print("featidx_t.get_shape() {}".format(featidx_t.get_shape()))
        feat_t = _embedding_lookup(featidx_t, self.feat_embedding)
        # [batch, feat_num*feat_dim]
        print("feat_t.get_shape() {}".format(feat_t.get_shape()))
    ...
    def decode_mode(self, state_t_1, context_input_t_1, context_concept_t_1, wordidx_t, featidx_t,
        ...
        with variable_scope.variable_scope("attention_decoder"):
            ...
            #print("vocab_scores {}".format(vocab_scores.get_shape()))
            #print("vocab_scores {}".format(tf.shape(vocab_scores)))
            #print("vocab_scores {}".format(tf.shape(vocab_scores)[-1]))
            topk_log_probs, topk_ids = \
                tf.nn.top_k(vocab_scores, min(options.topk_size, vocab_scores.get_shape()[-1])) # calculate topK
